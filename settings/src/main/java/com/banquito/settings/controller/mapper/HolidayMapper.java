...
public class HolidayMapper {
    ...
    public static Holiday toHoliday(HolidayRQ rq) throws ParseException {
       ...
       return Holiday.builder().date(dateFinal).name(rq.getName().toUpperCase()).codigoLocation(rq.getCode()).type(rq.getType()).build();
       //return Holiday.builder().date(rq.getDate()).name(rq.getName()).codigoLocation(rq.getCode()).type(rq.getType()).build();
    }

    public static HolidayRS toHolidayRS(Holiday holiday) {
        return HolidayRS.builder().date(holiday.getDate()).code(holiday.getCodigoLocation()).name(holiday.getName().toUpperCase()).type(holiday.getType()).build();
    }
}
