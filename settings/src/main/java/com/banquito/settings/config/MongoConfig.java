package com.banquito.settings.config;
...
public class MongoConfig extends AbstractMongoClientConfiguration {
    ...
    public MongoClient mongoClient() {
        if ("1".equals(appValues.getMongoAut())) {
            return MongoClients.create("mongodb+srv://" + appValues.getMongoUsr() + ":" + appValues.getMongoPwd() + "@" 
                    + "settings-cluster.rl6mawu.mongodb.net/" + appValues.getMongoDB() + "?retryWrites=true&w=majority");
        } else {
            return MongoClients.create("mongodb://" + appValues.getMongoHost() + ":8081/" + appValues.getMongoDB());
        }
    }
    ...
    public MappingMongoConverter mappingMongoConverter(
        MongoDatabaseFactory databaseFactory, MongoCustomConversions customConversions, MongoMappingContext mappingContext
    ) {
